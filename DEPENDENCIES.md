...
`constraints/windows-release.txt`
...
then run `scripts/verify_constraints.py` against the installed environment and `pip check`.

The verifier requires exact snapshot closure (missing and unexpected packages both fail). This prevents a missing direct/transitive dependency from being silently resolved outside the reviewed artifact.
